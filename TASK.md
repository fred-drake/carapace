...
- [x] **[BUILD]** Add `tsc-alias` (or equivalent) to the build pipeline so
...
- [x] **[TOOLING]** Update `packageManager` in `package.json` from
      `pnpm@9.15.4` to match Nix dev shell version (currently `pnpm@10.28.0`).
      Prevents `corepack` version mismatch conflicts.
- [x] **[TYPES]** Create `tsconfig.check.json` that extends `tsconfig.json`
...
- [x] **[SAFETY]** Add `"private": true` to `package.json` to prevent
      accidental npm publish during early development.

## 🟢 Low Priority

- [x] **[GIT]** Add `.DS_Store` to `.gitignore` (macOS development artifact).
- [x] **[COVERAGE]** Add `html` to Vitest coverage reporters for local visual
